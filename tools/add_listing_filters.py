...
#mhFilters .mh-chip{ display:inline-flex; align-items:center; white-space:nowrap; padding:9px 20px; border:1.5px solid #004d71; border-radius:999px; background:#fff; color:#004d71; font-weight:600; font-size:14px; line-height:1.2; cursor:pointer; user-select:none; transition:background .15s,color .15s,box-shadow .15s; }
...
(function(){
    ...
    function init(){
        ...
        var items=cards.map(function(card,i){
            ...
            var revEl=card.querySelector('.text-golden'); var idEl=card.querySelector('[data-doctorid],[data-doctorId]');
            var did=idEl?(idEl.getAttribute('data-doctorid')||idEl.getAttribute('data-doctorId')):'';
